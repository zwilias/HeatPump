...
bool HeatPump::connect(SoftwareSerial *serial, int rx, int tx) {
	return connect(serial, 0, rx, tx);
}

bool HeatPump::connect(SoftwareSerial *serial, int bitrate, int rx, int tx) {
  if(serial != NULL) {
    _HardSerial = serial;
  }
  bool retry = false;
  ...
  if (rx >= 0 && tx >= 0) {
#if defined(ESP32)    
    _HardSerial->begin(bitrate, SWSERIAL_8E1, rx, tx);
#else
    _HardSerial->begin(bitrate, SWSERIAL_8E1);
#endif    
  } else {
    _HardSerial->begin(bitrate, SWSERIAL_8E1);
  }
